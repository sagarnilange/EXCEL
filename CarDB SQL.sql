...
select* from Cars
where  price > (select avg(price) from Cars where fueltype = 'Gasoline') and fueltype = 'Gasoline';
...
select*  from Cars
where price > (select avg(price) from Cars where model = 2021) and model = 2021;

4.	Retrieve the details of cars with prices greater than the average price;
...
5.	Retrieve the brand and model of the car with the highest mileage;
...
6.Find the brand with the most car models in the dataset;

select brand, count(model) from Cars
group by brand
order by count(model) desc limit 1;

7.	Retrieve cars with prices closest to the average price, rounded to the nearest hundred;

select avg(price) from cars;
select round(avg(price),-2) from cars;

select * from cars where round(price,-2) = 57200 ;

8.	Find the model of the cheapest car among those with a V8 engine;

select CarID, Brand, model from cars
where enginetype = 'V8'
order by price asc limit 1;

9.	Retrieve brands with more than two car models in the dataset;

select brand, count(model) from cars
group by brand
having count(model) > 2;

10.	List cars with prices higher than the average price of cars with the same transmission type and fuel type;

select * from cars c
where  c.price > (select avg(c2.price) from cars c2
where c2.TransmissionType = c.TransmissionType and c2.fueltype = c.fueltype);

11.	Find the brand with the highest total mileage across all models.
